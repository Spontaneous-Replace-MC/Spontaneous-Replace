...
public class Block extends HorizontalFacingBlock {
    ...
    public Block(Settings settings) {
        ...
        setDefaultState(getDefaultState()
                .with(CHRYSALIS_STYLE, ChrysalisStyle.SYSTEM_SET)
    ...
    public static ChrysalisStyle getRandomStyle() {
        ...
        if (random > 77)        // 22%
            return ChrysalisStyle.DEFAULT;
        else if (random > 55)   // 22%
            return ChrysalisStyle.LARGE;
        else if (random > 33)   // 22%
            return ChrysalisStyle.SMALL;
        else if (random > 21)   // 12%
            return ChrysalisStyle.HUMANOID;
        else if (random > 11)   // 10%
            return ChrysalisStyle.VILLAGER;
        else if (random > 1)    // 10%
            return ChrysalisStyle.CHICKEN;
        else if (random > 0)    // 1%
            return ChrysalisStyle.CREEPER;
        else                    // 1%
            return ChrysalisStyle.IRON_GOLEM;
    }
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if (state.get(Properties.VERTICAL_DIRECTION) == Direction.UP) {
            return switch (state.get(CHRYSALIS_STYLE)) {
                case SYSTEM_SET, PLACEHOLDER, IRON_GOLEM, HUMANOID, VILLAGER, CREEPER ->
