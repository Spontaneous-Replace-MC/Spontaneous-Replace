...
public abstract class SRCrossbow extends RangedWeaponItem implements Vanishable {
    public static final double DEFAULT_USING_SPEED = 0;
     ...
     * @param range               范围
     */
    public SRCrossbow(Settings settings, Predicate<ItemStack> predicate, double usingSpeed, double maxUseTime, double maxBulletSpeed, double basicDamageMultiple, int range) {
        super(settings);
     ...
     * @param predicate 可使用弹药
     */
    public SRCrossbow(Settings settings, Predicate<ItemStack> predicate) {
        super(settings);
        PROJECTILES = predicate;
    ...
    public int getPullTime(ItemStack stack) {
        ...
        return quickChargeLevel == 0 ? MAX_USE_TICKS : MAX_USE_TICKS - MAX_USE_TICKS / 5 * quickChargeLevel;
    }
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (world.isClient) {
            ...
            double speedDValue = prevSpeed / speed;
            // 如果玩家在潜行则修改前行速度倍率
            ...
            if (Math.abs(USING_SPEED) > SRData.Player.SNEAK_SPEED) {
                if (speedDValue < 1.01 || Math.abs(prev2Speed - speed) < 0.2)
                ...
                if (speedDValue > 0.99 || Math.abs(prev2Speed - speed) < 0.2)
                    user.move(MovementType.SELF, user.getVelocity().multiply(speedMultiply, 1, speedMultiply));
            } else {
