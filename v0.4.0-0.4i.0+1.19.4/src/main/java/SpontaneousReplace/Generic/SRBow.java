...
public abstract class SRBow extends RangedWeaponItem implements Vanishable {
    public static final double DEFAULT_USING_SPEED = -1;
     ...
     * @param range               范围
     */
    public SRBow(Settings settings, Predicate<ItemStack> predicate, double usingSpeed, double maxUseTime, double maxPullTime, double maxBulletSpeed, double basicDamageMultiple, int range) {
        super(settings);
     ...
     * @param predicate 可使用弹药
     */
    public SRBow(Settings settings, Predicate<ItemStack> predicate) {
        super(settings);
        PROJECTILES = predicate;
     ...
     */
    public abstract float getBulletId();
}
