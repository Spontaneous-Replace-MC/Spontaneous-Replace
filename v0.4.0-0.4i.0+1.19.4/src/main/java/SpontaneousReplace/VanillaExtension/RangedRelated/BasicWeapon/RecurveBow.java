...
public class RecurveBow extends SRBow {
    ...
    public RecurveBow(Settings settings) {
        super(settings, RECURVE_BOW_PROJECTILES, USING_SPEED, DEFAULT_MAX_USE_TIME, MAX_PULL_TIME, MAX_ARROW_SPEED, BASIC_DAMAGE_MULTIPLE, RANGE);
    }
     ...
     * <p>耐久为弓的 2 倍</p>
     */
    public static final RecurveBow RECURVE_BOW = new RecurveBow(new Item.Settings().maxDamageIfAbsent(MAX_DAMAGE));
    ...
    public static void register() {
        // 注册反曲弓
        Registry.register(Registries.ITEM, new Identifier(MOD_ID, "recurve_bow"), RECURVE_BOW);
        ItemGroupEvents.modifyEntriesEvent(SRItemGroup.EQUIPMENT).register(content -> content.add(RECURVE_BOW));
        // 注册反曲弓谓词
        ...
        ModelPredicateProviderRegistry.register(RECURVE_BOW, new Identifier("arrow"), (stack, world, entity, seed) -> {
            if (entity == null)
                return 0;
            return entity.getActiveItem() != stack ? 0 : ((RecurveBow) stack.getItem()).getBulletId();
        });
    }
}
