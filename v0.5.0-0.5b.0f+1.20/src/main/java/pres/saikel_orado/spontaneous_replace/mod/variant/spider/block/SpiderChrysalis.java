...
/**
 * <b style="color:FFC800"><font size="+2">SpiderChrysalis：蜘蛛茧蛹方块类</font></b>
...
public class SpiderChrysalis extends HorizontalFacingBlock {
     ...
     * 蜘蛛茧蛹方块样式
     */
    protected SRSpiderData.ChrysalisStyle style = SRSpiderData.ChrysalisStyle.PLACEHOLDER;
    ...
    public SpiderChrysalis(Settings settings) {
        ...
        setDefaultState(getDefaultState()
                .with(SRSpiderData.CHRYSALIS_STYLE, SRSpiderData.ChrysalisStyle.PLACEHOLDER)
    ...
    protected void appendProperties(StateManager.Builder<net.minecraft.block.Block, BlockState> builder) {
        builder.add(SRSpiderData.CHRYSALIS_STYLE, Properties.HORIZONTAL_FACING, Properties.VERTICAL_DIRECTION);
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
            ...
            if (direction == Direction.UP) {
                if (!(blockPos.getY() < world.getTopY() - 1 && world.getBlockState(blockPos.up()).canReplace(ctx))) {
                    return Objects.requireNonNull(super.getPlacementState(ctx))
                            .with(SRSpiderData.CHRYSALIS_STYLE, SRSpiderData.ChrysalisStyle.SMALL)
        ...
        return Objects.requireNonNull(super.getPlacementState(ctx))
                .with(SRSpiderData.CHRYSALIS_STYLE, style)
                .with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing().getOpposite())
                .with(Properties.VERTICAL_DIRECTION, direction);
    }
     ...
     * @return 随机茧蛹风格
     */
    public static SRSpiderData.ChrysalisStyle getRandomStyle() {
        int random = new Random().nextInt(100);
        if (random > 77)        // 22%
            return SRSpiderData.ChrysalisStyle.DEFAULT;
        else if (random > 55)   // 22%
            return SRSpiderData.ChrysalisStyle.LARGE;
        else if (random > 33)   // 22%
            return SRSpiderData.ChrysalisStyle.SMALL;
        else if (random > 21)   // 12%
            return SRSpiderData.ChrysalisStyle.HUMANOID;
        else if (random > 11)   // 10%
            return SRSpiderData.ChrysalisStyle.VILLAGER;
        else if (random > 1)    // 10%
            return SRSpiderData.ChrysalisStyle.CHICKEN;
        else if (random > 0)    // 1%
            return SRSpiderData.ChrysalisStyle.CREEPER;
        else                    // 1%
            return SRSpiderData.ChrysalisStyle.IRON_GOLEM;
    }
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if (state.get(SRSpiderData.CHRYSALIS_STYLE) == SRSpiderData.ChrysalisStyle.PLACEHOLDER_SHORT) {
        ...
        if (state.get(Properties.VERTICAL_DIRECTION) == Direction.UP) {
            return switch (state.get(SRSpiderData.CHRYSALIS_STYLE)) {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (isDoubleBlock(state.get(SRSpiderData.CHRYSALIS_STYLE))) {
            ...
            if (state.get(SRSpiderData.CHRYSALIS_STYLE) == SRSpiderData.ChrysalisStyle.DEFAULT)
                world.setBlockState(blockPos, getDefaultState().with(SRSpiderData.CHRYSALIS_STYLE, SRSpiderData.ChrysalisStyle.PLACEHOLDER_SHORT).with(Properties.VERTICAL_DIRECTION, blockState), net.minecraft.block.Block.NOTIFY_ALL);
            else
                world.setBlockState(blockPos, getDefaultState().with(SRSpiderData.CHRYSALIS_STYLE, SRSpiderData.ChrysalisStyle.PLACEHOLDER).with(Properties.VERTICAL_DIRECTION, blockState), net.minecraft.block.Block.NOTIFY_ALL);
     ...
     * 是双高块
     */
    public static boolean isDoubleBlock(SRSpiderData.ChrysalisStyle style) {
        return style != SRSpiderData.ChrysalisStyle.SMALL && style != SRSpiderData.ChrysalisStyle.CHICKEN && style != SRSpiderData.ChrysalisStyle.PLACEHOLDER;
    }
